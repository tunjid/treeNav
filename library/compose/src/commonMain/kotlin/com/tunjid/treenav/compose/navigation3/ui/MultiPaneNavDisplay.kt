...
import com.tunjid.treenav.compose.navigation3.scene.LocalEntriesToExcludeFromCurrentScene
import com.tunjid.treenav.compose.navigation3.scene.SceneState
import com.tunjid.treenav.compose.navigation3.scene.rememberSceneState
...
internal object NavDisplay {
     ...
     * [MultiPaneNavDisplay] that the content should be animated using the provided [ContentTransform].
     *
    ...
    /**
     * Function to be called on the [NavEntry.metadata] or [Scene.metadata] to notify the
     * [MultiPaneNavDisplay] that, when popping from backstack, the content should be animated using the
    ...
    /**
     * Function to be called on the [NavEntry.metadata] or [Scene.metadata] to notify the
     * [MultiPaneNavDisplay] that, when popping from backstack using a Predictive back gesture, the content
 ...
 */
@Composable
internal fun <T : Any> MultiPaneNavDisplay(
    backStack: List<T>,
    ...
    val sceneToExcludedEntryMap =
        remember(sceneMap.entries.toList(), overlayScenes.toList(), zIndices.toString()) {
            buildMap {
                ...
                val scenesInZOrder = overlayScenes + scenes.sortedByDescending {
                    it.key == transition.targetState.key
                }
    ...
    if (inPredictiveBack) {
        if (transition.currentState != previousScene) {
            LaunchedEffect(previousScene, progress) {
                ...
                if (previousScene != null) transitionState.seekTo(progress, previousScene)
            }
        }
    } else {
